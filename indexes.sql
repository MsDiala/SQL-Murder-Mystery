...
-- Adding composite index to optimize Query 8 (Income analysis by car make)
CREATE INDEX idx_license_car_make ON drivers_license(car_make);

-- Adding index to speed up person-to-income joins for all economic queries
CREATE INDEX idx_income_ssn_lookup ON income(ssn);
