...
-- ➕ Additional suggested Indexes based on baseline analysis

-- Q8: JOIN drivers_license.id used in two large joins
CREATE INDEX idx_dl_id ON drivers_license(id);

-- Q2, Q6: drivers_license.id frequently joined
CREATE INDEX idx_dl_id_person ON drivers_license(id);

-- Q3: Optional composite to speed up date + membership join
CREATE INDEX idx_checkin_date_membership ON get_fit_now_check_in(check_in_date, membership_id);

-- Q5: Optional composite for faster filtering + join
CREATE INDEX idx_facebook_date_person ON facebook_event_checkin(date, person_id);
