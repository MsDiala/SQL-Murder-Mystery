...
**Scan Type:** SEARCH
**Notes:** Highly efficient. The engine uses a Composite Index (idx_crime_city_type_date) to filter both city and type simultaneously, avoiding a full table scan.
...
**Execution Time:** 58.634 ms
**Scan Type:** SCAN p, SEARCH dl
**Notes:** Performs a Full Scan on the person table using an index for sorting/ordering, followed by a Primary Key lookup on drivers_license. The high execution time (28ms) is due to the large number of rows (10,006) being joined.
